...
<div class="card-glass rounded-2xl p-5 mb-6 border border-violet-500/20 shadow-lg" data-aos="fade-up" data-aos-delay="50">
    <form method="GET" action="/payrollsystem/admin/leaves" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <div>
            <label class="block text-xs font-bold uppercase tracking-wider text-violet-300 mb-1.5">Search Employee</label>
            <input type="text" name="search" value="<?= htmlspecialchars($data['filters']['search']) ?>" placeholder="Name or employee code..." class="w-full px-3.5 py-2.5 bg-darker/70 border border-violet-700/30 text-white rounded-xl focus:ring-2 focus:ring-violet-500 text-xs shadow-inner">
        </div>
        <div>
            <label class="block text-xs font-bold uppercase tracking-wider text-violet-300 mb-1.5">Department</label>
            <select name="department_id" onchange="this.form.submit()" class="w-full px-3.5 py-2.5 bg-darker/70 border border-violet-700/30 text-gray-300 rounded-xl focus:ring-2 focus:ring-violet-500 text-xs shadow-inner cursor-pointer">
        ...
        <div>
            <label class="block text-xs font-bold uppercase tracking-wider text-violet-300 mb-1.5">Application Date</label>
            <input type="date" name="date" value="<?= htmlspecialchars($data['filters']['date'] ?? '') ?>" onchange="this.form.submit()" class="w-full px-3.5 py-2.5 bg-darker/70 border border-violet-700/30 text-white rounded-xl focus:ring-2 focus:ring-violet-500 text-xs shadow-inner">
        </div>
        <div class="flex items-end gap-2">
            <a href="/payrollsystem/admin/leaves" class="w-full py-2.5 text-center bg-surface hover:bg-violet-600/20 text-gray-300 border border-violet-900/40 font-bold rounded-xl text-xs transition-colors">
                Clear Filters
            </a>
        </div>
    </form>
...
<div class="card-glass rounded-3xl overflow-hidden border border-violet-500/20 mb-8 shadow-xl" data-aos="fade-up" data-aos-delay="100">
                ...
                <?php if(empty($data['leaveRequests'])): ?>
                    <tr>
                        <td colspan="6" class="px-6 py-12 text-center text-gray-500">
                            ...
                            <p class="font-semibold text-gray-300">No leave requests found</p>
                    ...
                    <tr class="hover:bg-violet-950/20 transition-colors group">
                        <td class="px-6 py-4">
                            <div class="flex items-center gap-3">
                                ...
                                <div>
                                    <div class="font-bold text-white group-hover:text-violet-300 transition-colors"><?= htmlspecialchars($lr['FirstName'] . ' ' . $lr['LastName']) ?></div>
                                    <div class="text-[11px] text-cyan-400 font-mono">EMP-<?= str_pad($lr['EmpID'], 4, '0', STR_PAD_LEFT) ?></div>
                                </div>
                            </div>
                        </td>
                        <td class="px-6 py-4">
                            <div class="font-bold text-white text-xs"><?= htmlspecialchars($lr['LeaveType']) ?></div>
                            <?php if($lr['IsPaid']): ?>
                        ...
                        <td class="px-6 py-4">
                            <div class="text-xs text-gray-300">
                                ...
                                <div class="font-extrabold text-amber-400 mt-0.5 font-mono"><?= $lr['days'] ?> Day(s)</div>
                            </div>
                        </td>
                        <td class="px-6 py-4 max-w-[200px] truncate text-gray-400 text-xs" title="<?= htmlspecialchars($lr['Reason']) ?>">
                        ...
                        <td class="px-6 py-4 text-right">
                            ...
                            <?php else: ?>
                                ...
                                <?php if(!empty($lr['admin_remark'])): ?>
                                    <div class="text-[10px] text-gray-400 italic mt-1 truncate max-w-[140px] ml-auto" title="<?= htmlspecialchars($lr['admin_remark']) ?>">"<?= htmlspecialchars($lr['admin_remark']) ?>"</div>
                                <?php endif; ?>
                            <?php endif; ?>
                        </td>
                    </tr>
    ...
    <div class="px-6 py-4 border-t border-violet-900/40 bg-surface/60 flex items-center justify-between text-xs">
        ...
        <div class="flex gap-1.5">
            <?php 
                ...
                $prevUrl = "?" . $queryString;
            ?>
            <a href="<?= $data['page'] > 1 ? $prevUrl : '#' ?>" class="px-3 py-1.5 border border-violet-700/30 rounded-xl bg-darker/60 text-gray-300 hover:bg-violet-600/20 transition-colors <?= $data['page'] <= 1 ? 'opacity-50 cursor-not-allowed' : '' ?>">Prev</a>
            
            <?php 
                $queryString = http_build_query(array_merge($_GET, ['page' => min($data['total_pages'], $data['page'] + 1)]));
                $nextUrl = "?" . $queryString;
            ?>
            <a href="<?= $data['page'] < $data['total_pages'] ? $nextUrl : '#' ?>" class="px-3 py-1.5 border border-violet-700/30 rounded-xl bg-darker/60 text-gray-300 hover:bg-violet-600/20 transition-colors <?= $data['page'] >= $data['total_pages'] ? 'opacity-50 cursor-not-allowed' : '' ?>">Next</a>
        </div>
    </div>
    <?php endif; ?>
</div>
...
<!-- Action Modal -->
<div id="actionModal" class="hidden fixed inset-0 z-[100] overflow-y-auto bg-gray-950/80 backdrop-blur-md flex items-center justify-center p-4 transition-all">
    <div class="card-glass rounded-3xl max-w-md w-full shadow-2xl overflow-hidden border border-violet-500/30 transform transition-all">
        <div class="px-6 py-5 border-b border-violet-900/40 flex justify-between items-center bg-surface/80">
            ...
            <button type="button" onclick="document.getElementById('actionModal').classList.add('hidden')" class="w-8 h-8 rounded-xl bg-surface text-gray-400 hover:text-white border border-violet-900/40 flex items-center justify-center transition-colors">
        ...
        <form action="/payrollsystem/admin/leaves" method="POST" class="p-6">
            ...
            <div class="mb-5">
                <label for="admin_remark" class="block text-xs font-bold uppercase tracking-wider text-violet-300 mb-2">Administrative Remarks (Optional)</label>
                <textarea name="admin_remark" id="admin_remark" rows="3" class="w-full px-4 py-3 bg-darker/70 border border-violet-700/30 text-white rounded-2xl focus:ring-2 focus:ring-violet-500 text-xs shadow-inner resize-none" placeholder="Add approval remarks or rejection rationale..."></textarea>
            </div>
