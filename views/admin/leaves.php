        ...
        <div>
            ...
            <select name="department_id" class="w-full px-3 py-2 bg-gray-50 border border-gray-300 rounded-lg focus:ring-primary focus:border-primary dark:bg-gray-700 dark:border-gray-600 dark:text-white text-sm">
                ...
                <?php foreach($data['departments'] as $dept): ?>
                    <option value="<?= $dept['DeptID'] ?>" <?= $data['filters']['DeptID'] == $dept['DeptID'] ? 'selected' : '' ?>><?= htmlspecialchars($dept['DeptName']) ?></option>
                <?php endforeach; ?>
            </select>
        </div>
        <div>
                    ...
                    <tr class="hover:bg-gray-50/50 dark:hover:bg-gray-700/50 transition-colors group">
                        <td class="px-6 py-3">
                            <div class="flex items-center gap-3">
                                ...
                                <div>
                                    <div class="font-bold text-gray-900 dark:text-white"><?= htmlspecialchars($lr['FirstName'] . ' ' . $lr['LastName']) ?></div>
                                    <div class="text-xs text-primary font-medium">EMP-<?= str_pad($lr['EmpID'], 5, '0', STR_PAD_LEFT) ?></div>
                                </div>
                            </div>
                        </td>
                        <td class="px-6 py-4">
                            <div class="font-medium text-gray-900 dark:text-gray-300"><?= htmlspecialchars($lr['LeaveType']) ?></div>
                            <?php if($lr['IsPaid']): ?>
                        ...
                        <td class="px-6 py-4 text-right">
                            <?php if($lr['Status'] === 'Pending'): ?>
                                <button onclick="actionLeave(<?= $lr['RequestID'] ?>)" class="inline-flex items-center bg-blue-50 text-blue-600 hover:bg-blue-100 dark:bg-blue-900/30 dark:text-blue-400 dark:hover:bg-blue-900/50 px-4 py-2 rounded-lg text-sm font-bold transition-colors shadow-sm">
