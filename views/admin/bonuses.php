...
<div id="addModal" class="hidden fixed inset-0 z-50 overflow-y-auto bg-black/50 backdrop-blur-sm flex items-center justify-center p-4">
    <div class="bg-surface dark:bg-gray-800 rounded-xl max-w-lg w-full shadow-2xl overflow-hidden transform transition-all">
    ...
    <input type="hidden" name="csrf_token" value="<?= $this->generateCsrfToken() ?>">
            ...
            <div>
                ...
                <select name="employee_id" id="employee_id" required class="w-full px-4 py-2 bg-surface/50 border border-gray-300 rounded-lg focus:ring-primary focus:border-primary dark:bg-gray-700 dark:border-gray-600 dark:text-white sm:text-sm transition-colors">
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="number" step="0.01" min="0.01" name="amount" id="amount" required class="w-full px-4 py-2 bg-surface/50 border border-gray-300 rounded-lg focus:ring-primary focus:border-primary dark:bg-gray-700 dark:border-gray-600 dark:text-white sm:text-sm transition-colors">
                </div>
                <div>
                    <label for="date" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Date</label>
                    <input type="date" name="date" id="date" required value="<?= date('Y-m-d') ?>" class="w-full px-4 py-2 bg-surface/50 border border-gray-300 rounded-lg focus:ring-primary focus:border-primary dark:bg-gray-700 dark:border-gray-600 dark:text-white sm:text-sm transition-colors">
                </div>
            </div>

            <div>
                <label for="type" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Bonus Type</label>
                <select name="type" id="type" required onchange="const ct = document.getElementById('custom_type_container'); const cti = document.getElementById('custom_type'); if(this.value === 'Other') { ct.classList.remove('hidden'); cti.setAttribute('required', 'required'); } else { ct.classList.add('hidden'); cti.removeAttribute('required'); }" class="w-full px-4 py-2 bg-surface/50 border border-gray-300 rounded-lg focus:ring-primary focus:border-primary dark:bg-gray-700 dark:border-gray-600 dark:text-white sm:text-sm transition-colors">
            ...
            <div id="custom_type_container" class="hidden">
                ...
                <input type="text" name="custom_type" id="custom_type" placeholder="e.g. Holiday Bonus" class="w-full px-4 py-2 bg-surface/50 border border-gray-300 rounded-lg focus:ring-primary focus:border-primary dark:bg-gray-700 dark:border-gray-600 dark:text-white sm:text-sm transition-colors">
            </div>

            <div>
                <label for="reason" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Reason / Description</label>
                <input type="text" name="reason" id="reason" required class="w-full px-4 py-2 bg-surface/50 border border-gray-300 rounded-lg focus:ring-primary focus:border-primary dark:bg-gray-700 dark:border-gray-600 dark:text-white sm:text-sm transition-colors">
            </div>

            <div class="flex justify-end gap-3 mt-6 pt-4 border-t border-gray-100 dark:border-gray-700">
                <button type="button" onclick="document.getElementById('addModal').classList.add('hidden')" class="px-5 py-2.5 text-sm font-medium text-gray-700 bg-surface border border-gray-300 rounded-lg hover:bg-gray-50 dark:bg-gray-800 dark:text-gray-300 dark:border-gray-600 dark:hover:bg-gray-700 transition-colors">Cancel</button>
